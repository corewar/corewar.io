...
describe('Fetcher', () => {
    ...
    it("fetches the next warrior's, next task's, next instruction", () => {
        ...
        state.instances = [TestHelper.buildWarriorInstance(), expectedWarrior]
        state.warriorIndex = 1

        const expectedTask = TestHelper.buildTask()

        expectedWarrior.tasks = [TestHelper.buildTask(), expectedTask, TestHelper.buildTask()]
    ...
    it('advances the correct warrior index, task index and instruction pointer', () => {
        ...
        state.instances = [TestHelper.buildWarriorInstance(), expectedWarrior, TestHelper.buildWarriorInstance()]
        state.warriorIndex = 1
    ...
    it('wraps the warrior index, task index and instruction pointer', () => {
        ...
        state.instances = [TestHelper.buildWarriorInstance(), TestHelper.buildWarriorInstance(), expectedWarrior]
        state.warriorIndex = 2

        const expectedTask = TestHelper.buildTask()

        expectedWarrior.tasks = [TestHelper.buildTask(), TestHelper.buildTask(), expectedTask]
    ...
    it('executes in the context of the next warrior if the current warrior has no tasks', () => {
        ...
        validWarrior.tasks = [TestHelper.buildTask()]

        const deadWarrior = TestHelper.buildWarriorInstance()
        deadWarrior.tasks = []

        state.instances = [validWarrior, deadWarrior]
    ...
    it(".getNextExecution returns new warrior's id and execution address", () => {
        ...
        expectedWarrior.tasks = [TestHelper.buildTask(), TestHelper.buildTask(), expectedTask, TestHelper.buildTask()]
        expectedWarrior.taskIndex = 2
