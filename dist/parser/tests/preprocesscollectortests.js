...
describe("PreprocessCollector", () => {
    it("Does not modify tokens if no EQU found", () => {
        ...
        var pass = new PreprocessCollector_1.PreprocessCollector();
    ...
    it("Removes EQU statements from the output", () => {
        ...
        var line2 = TestHelper_1.TestHelper.equ(2, "label1", [
            {
                category: IToken_1.TokenCategory.Label,
                lexeme: "label1",
    ...
    it("Raises a warning if a label is redefined", () => {
        var line1 = TestHelper_1.TestHelper.equ(1, "label1", [
            {
                category: IToken_1.TokenCategory.Opcode,
                lexeme: "MOV",
        ...
        var line2 = TestHelper_1.TestHelper.equ(2, "label2", [
            {
                category: IToken_1.TokenCategory.Opcode,
                lexeme: "ADD",
        ...
        var line3 = TestHelper_1.TestHelper.equ(3, "label1", [
            {
                category: IToken_1.TokenCategory.Opcode,
                lexeme: "SUB",
        ...
        expect(context.messages[0].type).toBe(IMessage_1.MessageType.Warning);
    });
    it("Takes the original EQU definition if a duplicate is found", () => {
        ...
        var line1 = TestHelper_1.TestHelper.equ(1, "label1", [movOpcode]);
        var line2 = TestHelper_1.TestHelper.equ(2, "label1", [addOpcode]);
        var context = new Context_1.Context();
    ...
    it("Does not include comments in EQU substitution", () => {
        var tokens = TestHelper_1.TestHelper.equ(1, "label1", TestHelper_1.TestHelper.instruction(1, "", "MOV", "", "", "0", ",", "", "1", "; this is a comment"));
        var context = new Context_1.Context();
        context.tokens = tokens.slice();
        var pass = new PreprocessCollector_1.PreprocessCollector();
        var result = pass.process(context, Parser_1.Parser.DefaultOptions);
    ...
    it("Collects multi-line EQU statements in ICWS-94draft", () => {
        var instruction1 = TestHelper_1.TestHelper.instruction(1, "", "MOV", "", "", "0", ",", "", "1", "");
        var instruction2 = TestHelper_1.TestHelper.instruction(2, "", "ADD", "", "", "1", ",", "", "2", "");
        // Remove terminating newlines
        instruction1.pop();
        instruction2.pop();
        var tokens = TestHelper_1.TestHelper.equ(1, "label2", instruction1)
            .concat(TestHelper_1.TestHelper.equ(2, "", instruction2));
        tokens.unshift({
            category: IToken_1.TokenCategory.Label,
            lexeme: "label1",
            position: { line: 1, char: 1 }
    ...
    it("Doesn't collect multi-line EQU statements in ICWS-88", () => {
        ...
        context.tokens = tokens.slice();
        var pass = new PreprocessCollector_1.PreprocessCollector();
        var result = pass.process(context, _.defaults({ standard: IParseOptions_1.Standard.ICWS88 }, Parser_1.Parser.DefaultOptions));
        var label1 = result.equs["label1"];
        var label2 = result.equs["label2"];
